...
colors = {
    "background": "#F0F4E3",     # 연한 국방 베이지
    "defense_green": "#4B5320",  # 진한 국방 녹색
    "olive_drab": "#556B2F",     # 강조용 짙은 올리브
    "dark_gray": "#2C3E50",      # 텍스트용 다크 그레이
    "accent_red": "#C62828",     # 포인트 빨강
}
...
st.markdown(
    ...
    /* 카드/위젯 배경 */
    .stMetric, .css-1r6slb0, .css-12oz5g7, .css-1lcbmhc {
    ...
    /* 사이드/탑 메뉴 버튼 */
    .nav-link, .css-1d391kg, .css-1v3fvcr {
        background: linear-gradient(90deg, #ff4b5c, #007bff) !important;
        color: white !important;
        border-radius: 8px;
    }

    /* 일반 텍스트 */
    .css-10trblm, .css-1offfwp, .css-16huue1 {
        color: white !important;
    }
    </style>
    """,
...
if 선택 == "메인":
    ...
    # 업체별 집계
    vendor_summary = df.groupby("낙찰자(상호)").agg(
        건수=("낙찰자(상호)", "count"),
        총낙찰금액=("낙찰금액", "sum")
    ).reset_index()
    ...
    while True:
        ...
        <div style="padding:20px; border-radius:20px; background-color:white;
            ...
            <div style="font-size:20px; font-weight:bold;">{vendor['낙찰자(상호)']}</div>
    ...
    if not final_df.empty:
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(widget_box("총 건수", f"{len(final_df):,}", "📊"), unsafe_allow_html=True)
        with col2:
        ...
        # 군수품 분류별 입찰 수
        # ----------------------------
        if "군수품분류" in final_df.columns:
            counts = final_df["군수품분류"].value_counts().reset_index()
            counts.columns = ["군수품분류", "건수"]

            order = ["1종", "2종", "3종", "4종", "5종",
                    "6종", "7종", "8종", "9종", "10종", "미분류"]

            counts["군수품분류"] = pd.Categorical(counts["군수품분류"], categories=order, ordered=True)
            counts = counts.sort_values("군수품분류")
            ...
            fig1 = px.bar(
                ...
                text="건수",
                color="군수품분류",
                category_orders={"군수품분류": order},
                color_discrete_sequence=px.colors.qualitative.Pastel
            )
            card_box("군수품 분류별 입찰 수", fig1)
...
with col1:
    st.image("C:/Users/user/Desktop/새 폴더/보람4조.png", width=400)
with col2:
    st.markdown("""
        ...
        <b>제작:</b> BORAM-IV | 
        <b>Version 1.0</b><br>
        (35222) 대전광역시 서구 월평중로2길 방위사업청 | 
        대표전화 : 1577-1118 | 팩스 : 02-773-7587
        </div>
    """, unsafe_allow_html=True)   
